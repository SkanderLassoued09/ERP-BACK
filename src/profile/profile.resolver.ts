...
export class ProfileResolver {
  ...
  async getTechWorkVolume(@Args('givenPrice') givenPrice: number) {
    ...
    dataDiag.forEach((diag) => {
      ...
      profile.numberDiDiag = diag.totalDiag.length;
      ...
      profile.totalDiagHours = Number((profile.totalDiag / 60).toFixed(2));
      profile.techCostDiag = Number(
        ((calculateTotalTime(diag.totalDiag) / 60) * givenPrice).toFixed(2),
      );
      //!TODO
      const rep = dataRep.forEach((r) => r.techName === diag.techName);
      if (rep) {
        profile.totalRep = calculateTotalTime(rep.totalRep);
        profile.numberDiRep = diag.totalRep.length;
        profile.totalRepHours = Number((profile.totalRep / 60).toFixed(2));
        profile.moyRep =
        ...
        profile.techCostRep = Number(
          ((calculateTotalTime(diag.totalRep) / 60) * givenPrice).toFixed(2),
        );
      }
      result.push(profile);
    });
